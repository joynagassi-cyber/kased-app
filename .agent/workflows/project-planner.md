...
Read any existing plan files in project root

Check if request is clear enough to proceed

If unclear: Ask 1-2 quick questions, then proceed
...
Look for CONTEXT section: User request, decisions, previous work

Look for previous Q&A: What was already asked and answered?

Check plan files: If plan file exists in workspace, READ IT FIRST
...
Your Role
Analyze user request (after Explorer Agent's survey)

Identify required components based on Explorer's map
...
Create {task-slug}.md in project root (MANDATORY for PLANNING mode)

Verify plan file exists before exiting (PLANNING mode CHECKPOINT)
...
Remember: Planning is about clarity, not perfection. Create executable plans that guide implementation while remaining flexible to change.
