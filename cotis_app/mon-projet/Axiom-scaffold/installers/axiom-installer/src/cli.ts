...
try {
  cli
    .command('', 'Interactive installer for Axiom-Scaffold — Add agentic skills to your AI coding tools')
    ...
    .action(async (options: Partial<CommandOptions>) => {
      if (isTTY) {
        printLogo()
        p.intro(`${c.inverse(` ${name}@${version} `)} ${c.dim('— Axiom-Scaffold Interactive Installer')}`)
      }
...
async function promptConfirm(skills: NpmSkill[], targetAgents: AgentType[]): Promise<void> {
  const result = await p.confirm({
    message: `🏗️  Install ${c.cyan(skills.length)} Axiom skill${skills.length > 1 ? 's' : ''} to ${c.cyan(targetAgents.length)} agent${targetAgents.length > 1 ? 's' : ''}?`,
  })
  if (p.isCancel(result) || !result) {
    p.outro(c.red('Installation cancelled'))
    process.exit(0)
  }
}
...
async function scanSkills(options: ResolvedOptions): Promise<NpmSkill[]> {
  ...
  spinner?.start('🔍 Scanning for Axiom skills...')
  ...
  if (skills.length === 0) {
    ...
    if (isTTY) {
      ...
      p.outro(c.dim('https://github.com/axiom-scaffold/axiom-installer'))
    }
    else {
      console.log(msg)
      if (hasInvalidSkills)
  ...
  if (isTTY)
    p.log.info('📦 Discovered Axiom skills:')
      ...
      if (options.yes) {
        ...
        const selected = await p.multiselect<string>({
          message: detectedAgents.length > 0
            ? '🎯 Select AI coding tools to install Axiom skills:'
            : '🎯 No agents detected. Select AI coding tools to install Axiom skills:',
